...
use log::debug;
use materials::{DiffuseMaterial, DisneyBRDFMaterial, SpecularMaterial};
...
fn main() {
    ...
    let bounding_box =
        BoundingBox::create(make_vec3(&[0.0, 0.0, 0.0]), make_vec3(&[5.0, 5.0, 5.0]));

    let ray = Ray::create(make_vec3(&[0.0, 0.0, 0.0]), make_vec3(&[1.0, 1.0, 1.0]));

    debug!(
        "Intersection with box is: {}",
        bounding_box.intersection(&ray)
    );
    ...
    let transformed_suzanne_mesh = transform_mesh(
        &(translate(0.0, 0.0, 0.0) * scale(2.0, 2.0, 2.0)),
        &suzanne_mesh,
    );
    let transformed_dragon_mesh = transform_mesh(
        &(translate(0.0, 0.0, 10.0) * scale(0.3, 0.3, 0.3)),
        &dragon_mesh,
    );
    ...
    */
    ...
    let disney_diffuse_material =
        DisneyBRDFMaterial::create(RGB::create(255.0, 120.0, 120.0), 0.0, 0.01, 0.8); //NOTE: names may no longer match material properties
    let disney_violet_diffuse_material =
        DisneyBRDFMaterial::create(RGB::create(40.0, 12.0, 148.0), 0.0, 0.9, 0.6);
    let disney_white_diffuse_material =
        DisneyBRDFMaterial::create(RGB::create(255.0, 255.0, 255.0), 0.0, 0.9, 0.02);
    let disney_red_diffuse_material =
        DisneyBRDFMaterial::create(RGB::create(255.0, 0.0, 0.0), 0.0, 0.9, 0.5);
    let disney_green_diffuse_material =
        DisneyBRDFMaterial::create(RGB::create(0.0, 255.0, 0.0), 0.0, 0.0, 0.5);
    let disney_blue_diffuse_material =
        DisneyBRDFMaterial::create(RGB::create(0.0, 0.0, 255.0), 0.0, 0.0, 0.5);
    let disney_yellow_diffuse_material =
        DisneyBRDFMaterial::create(RGB::create(180.0, 180.0, 0.0), 0.0, 0.9, 0.1);
    let disney_glossy_material =
        DisneyBRDFMaterial::create(RGB::create(255.0, 255.0, 0.0), 0.0, 0.5, 0.5);
    let disney_silver_material =
        DisneyBRDFMaterial::create(RGB::create(211.0, 211.0, 211.0), 0.2, 0.9, 0.02);
    ...
    let mut other_primitives = vec![
        ...
        //  Primitive::create(Box::new(*spherical_area_light.clone()), Box::new(white_diffuse_material.clone())),
