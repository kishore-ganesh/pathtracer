...
pub enum Object {
    ...
    Plane(Plane),
    Triangle(Triangle),
    SphericalAreaLight(SphericalAreaLight),
}

impl Object {
    pub fn intersection(&self, r: &Ray) -> Option<RayIntersection> {
        match self {
            Object::Sphere(sphere) => sphere.intersection(r),
            Object::Triangle(triangle) => triangle.intersection(r),
            Object::TriangleMesh(triangle_mesh) => triangle_mesh.intersection(r),
            Object::Plane(plane) => plane.intersection(r),
            Object::SphericalAreaLight(spherical_area_light) => {
                spherical_area_light.intersection(r)
            }
        }
    }
    pub  fn color(&self, v: &TVec3<f32>) -> RGB {
        match self {
            Object::Sphere(sphere) => sphere.color(v),
            Object::Triangle(triangle) => triangle.color(v),
            Object::TriangleMesh(triangle_mesh) => triangle_mesh.color(v),
            Object::Plane(plane) => plane.color(v),
            Object::SphericalAreaLight(spherical_area_light) => spherical_area_light.color(v),
        }
    }
    pub fn le(&self, p: &TVec3<f32>, v: &TVec3<f32>) -> RGB {
        match self {
            Object::Sphere(sphere) => sphere.le(p, v),
            Object::Triangle(triangle) => triangle.le(p, v),
            Object::TriangleMesh(triangle_mesh) => triangle_mesh.le(p, v),
            Object::Plane(plane) => plane.le(p, v),
            Object::SphericalAreaLight(spherical_area_light) => spherical_area_light.le(p, v),
        }
    }
    pub fn bounds(&self) -> BoundingBox {
        match self {
            Object::Sphere(sphere) => sphere.bounds(),
            Object::Triangle(triangle) => triangle.bounds(),
            Object::TriangleMesh(triangle_mesh) => triangle_mesh.bounds(),
            Object::Plane(plane) => plane.bounds(),
            Object::SphericalAreaLight(spherical_area_light) => spherical_area_light.bounds(),
        }
    }
}
...
impl Primitive {
    pub fn create(o: Object, m: Material) -> Self {
        return Primitive {
            object: o,
            material: m,
        };
    ...
    pub fn create_from_mesh(o: &TriangleMesh, m: Material) -> Vec<Self> {
        let mut v: Vec<Self> = vec![];
        debug!("Creating from mesh with {} triangles", o.mesh.len());
        for t in &o.mesh {
            v.push(Self::create(Object::Triangle(t.clone()), m.clone()));
        }
        return v;
    }
...
impl Sphere {
    ...
    pub fn intersection(&self, r: &Ray) -> Option<RayIntersection> {
        ...
        if b * b < 4.0 * a * c {
            ...
            // println!("Sphere intersection interm, time: {:?}",  duration.elapsed());
    ...
    pub fn le(&self, _: &TVec3<f32>, _: &TVec3<f32>) -> RGB {
        return RGB::black();
    }
